...
function moveAgent(action) {
    ...
    if (env.done) {
        alert('¡El episodio ha terminado! Reinicia el entorno.');
        return;
    }
...
function updateDisplay() {
    ...
    document.getElementById('status').textContent = env.done ? '¡Completado! ✓' : 'En progreso';

    // Deshabilitar botones si el episodio ha terminado
    const buttons = document.querySelectorAll('.action-btn');
    buttons.forEach(btn => {
        btn.disabled = env.done;
    });
}
...
// Crear entorno inicial al cargar la página
