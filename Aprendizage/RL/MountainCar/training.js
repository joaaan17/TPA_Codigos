...
async function startTraining() {
    ...
    for (let episode = 0; episode < episodes; episode++) {
        ...
        // Ejecutar episodio
        const visualize = (episode % 10 === 0); // Visualizar cada 10 episodios
        const result = await runTrainingEpisode(env, agent, visualize, speed);
        ...
        // Actualizar UI
        updateTrainingUI(episode + 1, episodes, result.totalReward, bestReward);
        
        // Log cada 50 episodios
        if ((episode + 1) % 50 === 0) {
            const avgLast100 = calculateAverageReward(100);
            const successRate = (successCount / (episode + 1) * 100).toFixed(1);
            addLog(`Episodio ${episode + 1}: Prom=${avgLast100.toFixed(1)}, Éxito=${successRate}%, ε=${agent.epsilon.toFixed(3)}`);
        }
...
function updateTrainingUI(episode, totalEpisodes, reward, bestReward) {
